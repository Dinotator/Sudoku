...
console.log('Llamada AJAX hecha.');
    ...
    if (bool) {
        ...
        if (!isValidGrid(grid)) {
            ...
            if (search(grid)) {
                ...
                alert("No existe solución alguna");
            }
        }
    }
}
...
function check_input() {
    ...
    for (var i = 0; i < 81; i++) {
        ...
        if (isNaN(arr[i])) {
            alert('Debe ser un dato del 1 al 9');
            return false
        }
    }
    ...
    if (search(grid)) {
        for (var i = 0; i < 81; i++) {
            if (grid[Math.floor(i / 9)][i % 9] != grid_original[Math.floor(i / 9)][i % 9]) {
                ...
                document.getElementsByTagName("input")[i].style.color = 'rgb(11, 4, 135)';
            }
        }
    }
